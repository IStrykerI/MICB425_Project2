...
![Nitrogen Cycle](N_Cycle_Figure.png)
...
kable(abun.dna.by.depth,caption="Normalized abundance of the norB gene (DNA) at different depths by RPKM")
```
...
abun.by.depth <- full_join(x = abun.dna.by.depth, 
                           y = abun.rna.by.depth, 
                           by = "Depth_m")
kable(abun.by.depth, caption="Normalized abundance of norB gene (DNA vs. RNA) at different depths by RPKM")
```
...
annotate_figure(plot1, bottom = text_grob("Figure 3: Normalized relative abundance (0-1) of norB gene (DNA vs. RNA) at different depths", size = 12))
```
...
kable(abun.by.class, caption="Taxa associated (including NA values) with normalized abundance of norB gene (DNA vs. RNA) at all depths by RPKM")
```

The total normalized abundances computed from DNA and RNA data is `r sum(abun.dna.by.class$Abundance)` and `r sum(abun.rna.by.class$Abundance)`, respictavely; however, only `r (sum(abun.dna.by.class.nonNA$Abundance))/sum(abun.dna.by.class$Abundance) * 100`$\%$ and `r (sum(abun.rna.by.class.nonNA$Abundance))/sum(abun.rna.by.class$Abundance) * 100`$\%$ have known genera for both DNA and RNA. We impute ``NA`` with ``unclassified`` for phylum and genus. 
...
According to Table 3, the genus associated with abundance of *norB* includes *Formosa*, *Muricauda*, *Zobellia*, *Achromatium*, *Beggiatoa*, *Candidatus Competibacter*, *Endozoicomonas*, *Gallionella*, *Thiocapsa*, and *Rhizobiales*. These genera belong to the three Phyla Bacteriodetes, Chlorobi and Proteobacteria. The abundance of phylum, as shown in Fig. 4, differed with respect to depth and with DNA versus RNA. For Proteobacteria, the abundance of *norB* was greater at 200m in comparison with 100m. At depth of 200m, greater abundance of RNA was observed in comparison with DNA.
...
annotate_figure(plot1, bottom = text_grob("Figure 5: Abundance of genera with norB (DNA vs. RNA) at different depths", size = 12))
```
...
annotate_figure(ggarrange(plot2, plot3, plot4, ncol = 3, nrow = 1, 
                                     labels = c("(a)", "(b)", "(c)")),
                bottom = text_grob("Figure 6: Abundance of norB in relation to nitrogen species across depths", size = 12))
```

According to Fig. 6, related to the abundance of NO~3~ and N~2~O, the abundance of *norB* increased from depth 10m to 100m, and declined from 100m to 200m. The abundance of *norB* related to NO~3~ has more of a sharp declined in comparison with that of N~2~O. Regarding to the abundance of *norB* related to NO~2~, a sharp declined is observed from 10m to 100m, the abundance level fluctuated from 100m to 150m, and declined from 150m to 200m.
...
15. TreeSAPP Demo by Connor Morgan-Lang & Kim Dill-McFarland.

16. Metagenomics. Date of Access: Apr.15.2018. Link: http://www.metagenomics.wiki/pdf/definition/rpkm-calculation. 
